...
#video_capture = cv2.VideoCapture(os.path.join(PATH_TO_IMAGE_UNKNOWN, 'video', 'MVI_0621.MOV'))
...
# TODO: for multiple photos for one person, add more known face encoding with known 
# face name similar to the old one. need more to test. image needed: 1 left, 
# 1 right, 1 w/o glass, 1 expsoe to light, 1 dark.
# IMPORTANT: len(known_face_encodings) == len(known_face_names) or at least 
# build a mapping function so that 1 known_face_encodings only has 1 names. 
